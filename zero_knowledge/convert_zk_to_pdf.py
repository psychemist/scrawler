...
def convert_to_pdf(book_dir, output_filename):
    ...
    # Pre-process markdown to handle images better if needed? 
    # fpdf2 write_html expects <img> src to be valid.
    # The markdown has "assets/..." which is correct if we run from book_dir.
    ...
    # Basic CSS for fpdf2
    # fpdf2 doesn't support full CSS but we can try basic tags.
    # We might need to sanitize HTML for fpdf2.
    ...
    try:
        ...
        # write_html might fail on some tags.
        # It handles <h1>, <p>, <img>, <ul>, <li>, <b>, <i>, <a>, <blockquote>, <pre>, <code> (maybe), <table> (basic)
        ...
        with open("../debug_book.html", "w", encoding='utf-8') as f:
            f.write(html)
        logger.info("Saved HTML to debug_book.html for manual conversion.")
    finally:
        os.chdir(cwd)

if __name__ == "__main__":
    convert_to_pdf("zk_book", "zk_book.pdf")
