...
def convert_to_pdf(book_dir, output_filename):
    ...
    # Ensure full path is used for checking existence if needed, but listdir returns names
    ...
    try:
        ...
        # Note: if pre_code_font causes issues in newer fpdf2, we might remove it, but mimicking existing script.
        try:
        ...
        with open("../debug_btc.html", "w", encoding='utf-8') as f:
            f.write(html)
        logger.info("Saved HTML to debug_btc.html for manual conversion.")
    finally:
        os.chdir(cwd)

if __name__ == "__main__":
    # Ensure we look for btc_book in the same directory as the script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    book_dir = os.path.join(script_dir, "btc_book")
    output_pdf = "btc_book.pdf" # This will be placed in script_dir because of the ".." in the function logic?
    # Wait, the function does `os.chdir(book_dir)` then outputs to `../output_filename`.
    # So if book_dir is learning-bitcoin/btc_book, output is learning-bitcoin/btc_book.pdf. Correct.
    
    convert_to_pdf(book_dir, output_pdf)
